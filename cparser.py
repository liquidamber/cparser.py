...
class CParser(object):
    ...
    def parse_paren_match(self):
        ...
        l = TokenList()
        if p.get_type() != "(":
            raise CParserSyntaxError, "Not OpenParen"+str(p)
        l.append(p)
        while True:
            a = self.cur_tok()
            if a.get_type() == "(":
                x = self.parse_paren_match()
                l.append(x)
            else:
                self.get_tok()
                l.append(a)
...
if __name__=="__main__":
    test(r'y = fuga ? x==5 : (++y + 20) * 8 & (4 >> 2 | 3) && org--')
    test(r'y = fuga ? x==sizeof(int)+5 : (y++ + 20) * 8 & (4 >> 2 | 3) && --*(double *)org')
